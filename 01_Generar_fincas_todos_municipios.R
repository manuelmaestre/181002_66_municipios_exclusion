...
file.geolocalizadas <- 'indata/direcciones_geolocalizadas.csv'
...
geolocalizadas <- geolocalizadas[!duplicated(cod_finca),]
geolocalizadas$geo_latitud <- as.double(geolocalizadas$geo_latitud)
geolocalizadas$geo_longitud <- as.double(geolocalizadas$geo_longitud)
geolocalizadas <- geolocalizadas[!is.na(geo_latitud),]

coordenadas.google <- as.matrix(geolocalizadas[,.(geo_longitud, geo_latitud)])
...
coordenadas.poblar$geo_latitud <- as.double(coordenadas.poblar$geo_latitud)

coordenadas.recuperadas <- merge(total.fincas, coordenadas.poblar, by.x = 'cod_finca', by.y = 'V1')
coordenadas.recuperadas$X <- NULL
coordenadas.recuperadas$Y <- NULL

setnames(coordenadas.recuperadas, c('geo_longitud', 'geo_latitud'), c('X', 'Y'))
setcolorder(coordenadas.recuperadas, colnames(total.fincas.xy))
coordenadas.recuperadas <- coordenadas.recuperadas[!cod_finca %in% total.fincas.xy$cod_finca,]

total.fincas.xy <- rbind(total.fincas.xy, coordenadas.recuperadas)
